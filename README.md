...
- **Single notification slot** — auto-scheduled to fire when lives are full, cancelled on refill / grant / max change
...
`ConsumeLife` returns `false` in **two** cases: when `Lives == 0`, and when `HasInfiniteLives` is active (count unchanged in the latter). Check `HasInfiniteLives` first so the "no lives" popup is not shown unnecessarily.
...
`TimeUntilNextLife` returns `TimeSpan.Zero` when full, during infinite, or before recovery has started — render it conditionally:
...
`FakeLivesStorage` and `FakeNotificationScheduler` ship in the `TMV.Lives.Tests.Shared` assembly and are reused by both EditMode and PlayMode test fixtures.
